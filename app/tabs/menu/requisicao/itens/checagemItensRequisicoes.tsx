...
type Props = {
  ...
  nrMov: string;
  onCancel: () => void;
  onUpdated?: (resp: any) => void; // opcional: para o pai atualizar a lista
};
...
export default function ItemCheckModal({ visible, item, nrMov, onCancel, onUpdated }: Props) {
  ...
  const confirmar = async () => {
    try {
      ...
      // regra: sn_check = "1" se qtd > 0
      const body = {
        cd_fil: String(filialSelecionada.cd_fil), 
        nr_mov: String(nrMov),
        cd_usu_check: String(usuarioSelecionada?.cd_usu ?? ''), // "008"
        sn_check: qtd > 0 ? '1' : '0',
        qt_check: String(qtd),
        // Se seu back também aceita time_check automático, pode omitir
      };
